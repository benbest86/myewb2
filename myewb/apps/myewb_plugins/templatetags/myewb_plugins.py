...
@inclusion_kwdtag(register, "myewb_plugins/app_plugin.html", takes_context=True)
def myewb_app_plugin(context, app, name, plugin=None, user=None, args=None,
               ext='.html', **extra_args):
    validate_name(app)
    validate_name(name)
    plugin_point=None
    if plugin is None:
        try:
            ...
            pass
        try:
            plugin_point = models.PluginPoint.objects.get(label=u'.'.join([app, name]))
    ...
    if plugin is None:
        ...
        if plugin_point is not None:
            nc.update(plugin_point.call(nc, user, **args))
        nc.push()
    else:
...
@inclusion_kwdtag(register, "myewb_plugins/widget.html", takes_context=True)
def widget(context, app, name, content_object, widget=None, user=None, args=None, ext='.html', **extra_args):
    validate_name(app)
    validate_name(name)
    plugin_point=None
    if widget is None:
        try:
            widget = models.Widget.objects.get(label=u'.'.join([app, name]))
    ...
    template = ''
    nc.update(widget.call(nc, user, content_object, **args))
    template = widget.template
    nc.update(args)
    nc.push()
    nc['widget_object'] = widget
    nc['widget_app'] = app
    nc['widget_plugin_point'] = name.split('.')[0]
    nc['widget_args'] = args
    nc['widget_user'] = user
    nc['widget_template'] = template
    return nc
...
def plugin_hook(context, name, point=None, user=None, ext='.html', **args):
    ...
    if point is None:
        try:
            # try to get point directly from input name - will only work with
            # plugin.registered points, not plugin_point.register
    ...
    plugins = None
    if point is None:
        # check cache for correct templates
        ...
        # nc is a RequestContext - pushing adds a blank dictionary to the context
        nc.push()
    else:
        ...
        if group:
            plugins += point.get_group_plugins(group)
...
def widget_hook(context, name, location, point=None, user=None, ext='.html', **args):
    ...
    widgets = None
    nc.update(point.call(nc, user, **args))
    widget_prefs = point.get_widget_prefs(user, location)
    # group = args.get('group', None)
    # if group:
    #     plugins += point.get_group_plugins(group)
    # apps = [ p.app for p in plugins ]
